...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
  ...
  if (isset($_POST['email']) && isset($_POST['password'])&& isset($_POST['key'])) {
    $get_user_email = $_POST['email'];

    $select_query = "select * from `admin_table` where admin_email= '$get_user_email'";
    $result = mysqli_query($conn, $select_query);
    $row_count = mysqli_num_rows($result);

    if ($row_count > 0) {
      $row_data = mysqli_fetch_assoc($result);
      $username = $row_data['admin_email'];
      $password = $row_data['password'];
      $security_key = $row_data['security_key'];
      $_SESSION['admin_id'] = $row_data['admin_id'];


      // Simulate database check (replace with actual database query)
      if ($_POST['email'] === $username && $_POST['password'] === $password && $_POST['key'] === $security_key) {

        $_SESSION['email'] = $_POST['email'];
        echo "<script>alert('login successfull')</script>";
        header("Location: admin.php"); // Redirect to the secured page
        exit();
      } else {
